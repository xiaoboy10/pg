...
class Spider(Spider):
	...
	def homeContent(self, filter):
		...
		url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Users/{embyInfos['User']['Id']}/Views"
		params = {
	...
	def categoryContent(self, cid, page, filter, ext):
		...
		header = self.header.copy()
		header['Content-Type'] = "application/json; charset=UTF-8"
		url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Users/{embyInfos['User']['Id']}/Items"
		params = {
			"X-Emby-Client": embyInfos['SessionInfo']['Client'],
			"X-Emby-Device-Name": embyInfos['SessionInfo']['DeviceName'],
			"X-Emby-Device-Id": embyInfos['SessionInfo']['DeviceId'],
		...
		for video in videoList:
			...
			videos.append({
				"vod_id": video['Id'],
				"vod_name": name,
				"vod_pic": f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Items/{video['Id']}/Images/Primary?maxWidth=400&tag={video['ImageTags']['Primary']}&quality=90" if 'Primary' in video['ImageTags'] else '',
	...
	def detailContent(self, did):
		...
		header = self.header.copy()
		header['Content-Type'] = "application/json; charset=UTF-8"
		url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Users/{embyInfos['User']['Id']}/Items/{did[0]}"
		params = {
			"X-Emby-Client": embyInfos['SessionInfo']['Client'],
			"X-Emby-Device-Name": embyInfos['SessionInfo']['DeviceName'],
			"X-Emby-Device-Id": embyInfos['SessionInfo']['DeviceId'],
		...
		vod = {
			...
			"vod_pic": f'http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Items/{did[0]}/Images/Primary?maxWidth=400&tag={videoInfos["ImageTags"]["Primary"]}&quality=90' if 'Primary' in videoInfos['ImageTags'] else '',
		...
		if not videoInfos['IsFolder']:
			...
			url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Shows/{did[0]}/Seasons"
			params.update(
				{
			...
			if r.status_code == 200:
				...
				for playInfo in playInfos:
					url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Shows/{playInfo['Id']}/Episodes"
					params.update(
						{
	...
	def searchContentPage(self, keywords, quick, page):
		...
		for vod in vodList:
			...
			pic = f'http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Items/{sid}/Images/Primary?maxWidth=400&tag={vod["ImageTags"]["Primary"]}&quality=90' if 'Primary' in vod["ImageTags"] else ''
			videos.append({
				"vod_id": sid,
				"vod_name": name,
				"vod_pic": pic,
	...
	def playerContent(self, flag, pid, vipFlags):
		...
		header = self.header.copy()
		header['Content-Type'] = "application/json; charset=UTF-8"
		url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Items/{pid}/PlaybackInfo"
		params = {
			"UserId": embyInfos['User']['Id'],
			"IsPlayback": "false",
		...
		url = f'http://127.0.0.1:9966/api/proxy/{quote(url)}'
	...
	def getAccessToken(self):
		key = f"emby_http://127.0.0.1:9966/api/proxy/{self.baseUrl}_{self.username}_{self.password }"
		...
		header = self.header.copy()
		header['Content-Type'] = "application/json; charset=UTF-8"
#		r = requests.post(f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Users/AuthenticateByName", params={"Username": self.username, "Password": self.password , "Pw": self.password , "X-Emby-Client": "Yamby", "X-Emby-Device-Name": "Yamby", "X-Emby-Device-Id": str(uuid4()), "X-Emby-Client-Version": "1.0.2"}, headers=header, timeout=120, proxies={"http": self.proxy, "https": self.proxy}
		r = requests.post(f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Users/AuthenticateByName", params={"Username": self.username, "Password": self.password , "Pw": self.password , "X-Emby-Client": "Emby Theater", "X-Emby-Device-Name": "DESKTOP-C25AFR6", "X-Emby-Device-Id": str(uuid4()), "X-Emby-Client-Version": "3.0.20-3.0"}, headers=header, timeout=120, proxies={"http": self.proxy, "https": self.proxy})
		embyInfos = r.json()
